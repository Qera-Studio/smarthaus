...
| Item                                           | Status                                                                                                                                  | Action                                                                                                                        |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Geist fonts are placeholder                    | Layout.tsx uses Geist Sans/Mono (Next.js defaults). Brief calls for serif/geometric sans headings + geometric sans body                 | Replace when final fonts are confirmed. Current CSS variables `--font-geist-sans` / `--font-geist-mono` are temporary         |
...
| Colour tokens are unnamed/unsemantic           | `_variables.scss` uses palette names (`--ink`, `--bronze`) not semantic names (`--color-bg-canvas`, `--color-text-primary`)             | Restructure to L0/L1 token layers (primitives → semantics) during architecture implementation                                 |
| `.env.example` is incomplete                   | Missing Sanity, Resend keys, WhatsApp number                                                                                            | Update when each integration is implemented. Turnstile deferred — add when spam requires it                                   |
...
| Per-page OG images not built                   | Default `opengraph-image.png` only. Inner pages need dynamic OG via `opengraph-image.tsx` (ImageResponse API) or static Blender renders | Part of launch gate. Being handled separately                                                                                 |
